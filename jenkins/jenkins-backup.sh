...
# Modified from https://github.com/sue445/jenkins-backup-script

readonly DEST_FILE="$1"
readonly JENKINS_HOME="${2-$JENKINS_HOME}"
...
function usage() {
  echo "usage: $(basename $0) archive.tar.gz [/path/to/jenkins_home]"
}
...
function main() {
  ...
  if [ -d "${JENKINS_HOME}/secrets/" ] ; then
    rsync -av --exclude master.key "${JENKINS_HOME}/secrets/" "${ARC_DIR}/secrets"
  fi
